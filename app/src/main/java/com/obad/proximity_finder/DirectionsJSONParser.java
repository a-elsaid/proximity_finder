...
public class DirectionsJSONParser {
    ...
    public List<List<HashMap<String,String>>> parse( JSONObject jObject ) { List<List<HashMap<String, String>>>
        routes = new ArrayList<List<HashMap<String,String>>>( ) ;
        JSONArray jRoutes = null;
        JSONArray jLegs = null; JSONArray jSteps = null;
        Object jStatus = null;
        try {
            jRoutes = jObject.getJSONArray( "routes" );
            jStatus = jObject.get("status");
            Log.d(TAG, "parse STATUS: " + jStatus);
    ...
    private List<LatLng> decodePoly( String encoded ) { List<LatLng> poly = new ArrayList<LatLng>( );
        ...
        while ( index < len ) {
            int b, shift = 0, result = 0; do {
                b = encoded.charAt( index++ ) - 63; result |= ( b & 0x1f ) << shift;
                shift += 5;
            } while ( b >= 0x20 );
